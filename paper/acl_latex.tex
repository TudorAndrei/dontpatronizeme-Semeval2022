...
\begin{enumerate}
	...
	\item Subtask 1 (Binary Classification)
        ...
        Since we experimented with various techniques and approaches, we decided to split the results based on the experiments made.
	      ...
	      \begin{enumerate}
		      ...
		      Logistic Regression gave solid results on all variations of the datasets, providing an \textit{f1\_score} of 0.35 on lemmatized dataset and 0.38 on stemmed validation dataset. Neural Networks provided as well good results, but did not manage to obtain the performance of Logistic Regression. We could infer from the tables that Logistic Regression gave the best performance on stemmed dataset.
		        ...
		        \begin{table*}
        \centering
        \caption{\label{table1} Results on Imbalanced and Oversampled Lemmatized \& Stemmed dataset. The results are in terms of \textit{f1\_score}. }
        \subcaption*{Results on Imbalanced and Oversampled Lemmatized dataset}
        \begin{tabular}{lrrrl}
        ...
        \end{tabular}
        \bigskip
        \subcaption*{Results on Imbalanced and Oversampled Stemmed dataset}
        \begin{tabular}{lrrrl}
        \hline \textbf{Approach $\backslash$ Dataset} & Simple & SPE & SMOTE & SVM-SMOTE\\ \hline
        ...
        \end{table*}
		      ...
		      \item Keras Tokenizer \& Word2Vec Embeddings + LSTM
		            ...
		            Another experiment that we conducted was the use of Keras Tokenizer and Word2Vec in order to extract the embeddings from the texts. We then applied two LSTM models: one with 64 neurons and the other one with 128 neurons.
		            
		            The results of these two models on both Lemmatized \& Stemmed datasets with two variations of created embeddings (Keras Tokenizer and Word2Vec) are provided in table \ref{table2}. LSTM with 64 neurons provided best results on the datasets that were using the default Tokenizer from Keras, with an f1\_score of almost 27\% on Lemmatized dataset and 32\& on Stemmed dataset. Word2Vec did not seem to provide good results in combination with LSTM networks.
        ...
        \subcaption*{Results on Lemmatized dataset with class weight}
        \begin{tabular}{lrl}
        ...
        \end{tabular}
        \bigskip
        \subcaption*{Results on Stemmed dataset with class weight}
        \begin{tabular}{lrl}
        \hline \textbf{Approach $\backslash$ Dataset} & Keras Tokenizer & Word2Vec\\ \hline
        ...
        \end{table*}
		      ...
		      The results are present in table \ref{table3}. We can infer that the third method provided the best results, but still insufficient to reach the level and performance of Logistic Regression from (a).
	...
	\item Subtask 2 (Multi-label Multi-class Classification)
	      ...
	      \begin{enumerate}
		      % TODO:  Please explain more the results, bold the best result and so on...
		      \item BERT models approach for classification across 7 classes.
                    Table \ref{table4}a) shows that the model was able to learn only two of the classes.
                    
                    \begin{table*}
        \centering
        \caption{\label{table4} Results on Imbalanced and Oversampled Lemmatized \& Stemmed dataset. The results are in terms of \textit{f1\_score}. }
        \subcaption*{a) Results on Imbalanced and Oversampled Lemmatized dataset}
        \begin{tabular}{lrrrrrrrl}
        \hline \textbf{Model $\backslash$ Class} & Unb & Sha & Pre & Aut & Met & Com & Mer & Mean Score\\ \hline
        ...
        \end{tabular}
        \bigskip
        \subcaption*{b) Results on Imbalanced and Oversampled Lemmatized dataset}
        \begin{tabular}{lrrrrrrrrrl}
        \hline \textbf{Model $\backslash$ Sub-classes} & Expert & Aut & Pre & Saviour & Sha & Unb & Poet & Com & Mer & Met\\ \hline
        BERT & 0.44 & 0.0 & 0.0 & 0.85 & 0.0 & 0.84 & 0.69 & 0.0 & 0.0 & 0.59 \\
        DistilRoBERTa & 0.54 & 0.0 & 0.0 & 0.85 & 0.0 & 0.84 & 0.69 & 0.11 & 0.0 & 0.65 \\
        DistilBERT & 0.42 & 0.0 & 0.40 & 0.75 & 0.0 & 0.81 & 0.61 & 0.0 & 0.0 & 0.67\\
        DistilBERTMLC & 0.36 & 0.0 & 0.0 & 0.86 & 0.0 & 0.83 & 0.60 & 0.0 & 0.0 & 0.52 \\
        \hline
        \end{tabular}
        ...
        \end{table*}
		      ...
		      \item The general class approach is detailed in table \ref{table4}b).
...
% TODO: Write about the best result for multi-label classification
